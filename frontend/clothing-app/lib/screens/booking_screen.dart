...
class _BookingScreenState extends State<BookingScreen> with TickerProviderStateMixin {
  ...
  void _calculateTotalCost() {
    if (_startDate != null && _endDate != null) {
      setState(() {
        ...
        final dailyPrice = double.tryParse(widget.item['price_per_day']?.toString() ?? '0') ?? 0.0;
        _totalCost = (_rentalDays * dailyPrice).toDouble();
        
        // No deposit - simple pricing
        _depositAmount = 0.0;
      });
    }
  }
  ...
  Future<void> _createBooking() async {
    ...
    try {
      ...
      final rentalData = await ApiService.createRental(
        rentalDate: DateFormat('yyyy-MM-dd').format(_startDate!),
        returnDate: DateFormat('yyyy-MM-dd').format(_endDate!),
        notes: 'Size: $_selectedSize',
        items: [
          {
      ...
      // Navigate to payment screen with rental data
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => PaymentScreen(
            rentalData: {
              ...rentalData,
              'rental_id': rentalData['rental']['id'],
              'total_amount': rentalData['rental']['total_amount'],
              'items': [
                {
                  ...widget.item,
                  'quantity': 1,
                  'price_per_day': widget.item['price_per_day'],
                }
              ],
            },
          ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (_rentalDays > 0) ...[
            ...
            _buildCostRow(
              'Total Amount',
              '${_totalCost.toStringAsFixed(2)} MAD',
              isTotal: true,
            ),
          ] else ...[
